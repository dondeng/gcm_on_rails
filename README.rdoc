...
gcm_on_rails is a Ruby on Rails gem that allows you to easily incorporate Google's 'Google Cloud Messaging for Android'
into your Rails application. This gem was derived from c2dm_on_rails (https://github.com/pimeys/c2dm_on_rails) after
Google deprecated C2DM on June 27, 2012

h2. Acknowledgements:

This gem was derived from Julius de Bruijn's gem c2dm_on_rails which according to him was originally a rewrite of the
gem apn_on_rails written by Mark Bates and before him Fabien Penso and Sam Soffes. Thanks to all the above who were
involved in earlier versions or derivatives of the gem. I originally wanted to use the c2dm_on_rails gem for a project
at Kopo Kopo, Inc when the rug was pulled from under me when Google pulled C2DM in favor of GCM (Google Cloud Messaging).
I thus decided to create an updated gem. This is also my first rubygem so please feel free to fix or add on anything you
may see fit.

h2. Requirements:

You will have to sign up to GCM first via the Google APIs Console page (https://code.google.com/apis/console).
When you greate an new API project, the browser url will change to something like:
...
Some settings need to be loaded for configuration purposes. An initialzer script that can be pu in config/initializers/gcm_on_rails.rb
...
The rake task will look for any unsent notifications from in the database. If found, the notifications will then be dispatched
...
|Code 401| |
| |API Key is Invalid. Check the configatron file/|
...
- Implement "broadcasting" sending and processing responses to multiple registration id's within one request. Currently only one
  message to a single registration id is implemented.
